...
while max(abs(delta_x)) > tolerance and counter < 5:
    ...
    # Update Theta Values
    delta_theta = np.linalg.solve(G, np.dot(H.T, np.dot(W, (z-h).T)))[:len(buses) - 1]
    x_theta += delta_theta
    ...
    for bus in buses.values():
        if bus.bustype == 'Vθ':
            ...
            bus.refresh(bus.V, x_theta[bus.ID - vtheta - 1])
    ...
    for bus in buses.values():
        ...
        # Save new calculated values of power
        bus.save_power(p, q)
    ...
    # Assembling submatrix h
    h = np.hstack((h_p, h_q, h_v))

    # Update V Values
    delta_V = np.linalg.solve(G, np.dot(H.T, np.dot(W, (z - h).T)))[len(buses) - 1:]
    x_V += delta_V

    for bus in buses.values():
        bus.refresh(x_V[bus.ID - 1], bus.theta)

    for bus in buses.values():
        print(bus.ID, bus.V, bus.theta)
